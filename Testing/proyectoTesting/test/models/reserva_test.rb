...
class ReservaTest < ActiveSupport::TestCase
  ...
  test 'Reserva creada con sala inexistente' do
    reserva = Reserva.create(sala: 15, fecha: Date.new(2022, 10, 11), asiento: 10, horario: 'TANDA',
                             name: 'Pedro')
    assert_equal(false, reserva.valid?)
  end

  test 'Reserva creada con asiento invalido' do
    reserva = Reserva.create(sala: 5, fecha: Date.new(2022, 10, 11), asiento: 100, horario: 'TANDA',
                             name: 'Pedro')
    assert_equal(false, reserva.valid?)
  end

  test 'Reserva creada sin nombre' do
    reserva = Reserva.create(sala: 15, fecha: Date.new(2022, 10, 11), asiento: 10, horario: 'TANDA')
    assert_equal(false, reserva.valid?)
  end
end
